# 暮らしのカルテ（kurashi-carte.jp）— 暮らし・住まいの悩みを診断で見える化する総合メディア

**母屋（総合ハブ）＝入口**。配下に診断コンテンツをサブディレクトリで無限に量産するプラットフォーム。
母屋トップは暮らし・住まいの悩みを持つ生活者を広く集め、各診断サブ（→D2C定期購入）へ分配する。
1顧客を複数診断で回遊させ、グループD2C全体でLTVを刈る（例：ダニ診断→洗濯診断→からだのカルテ）。

- **母屋（`index.html`）**：GEO直接回答＋「暮らしのカルテとは」＋診断カード（`config.json` の `home.contents` をループ）＋LINE導線。
- **診断サブ**：`/sentaku/`（洗濯・水回り→マグちゃん・公開中）、`/dani/`（住まいのダニ・アレルギー→Apros・近日公開）… 以降 config に1件足すだけで母屋に自動掲載。
- **運営**：リブメーカーズ株式会社（© LIVmakers co.,ltd.）。商品ブランド：マグちゃん（magchan.com）ほかグループD2C。

各診断が回す**3層キャッシュマシン**（マグちゃん例）：
...
## ＋ 診断サブを増やす（母屋に自動掲載＝この器の核心）
新しい診断を作ったら、**`config.json` の `home.contents` 配列に1件足すだけ**で母屋トップの診断カードに自動で載ります（ハードコードしない＝量産拡張性）。

```json
{
  "id": "kabi",                      // 任意のID
  "status": "soon",                  // "live"=リンク有効 ／ "soon"=近日公開（非リンク・グレー表示）
  "badge": "近日公開",               // カード左上タグ
  "title": "お風呂のカビ・湿気診断",
  "desc": "……（1〜2文の説明）",
  "url": "/kabi/",                   // サブディレクトリ
  "icon": "/assets/type-xxxx.svg",   // 76px想定のアイコンSVG（assetsにローカル同梱）
  "cta": "無料で診断する",
  "connect": "○○（接続先D2C）へ接続" // 任意
}
```

- **公開の切替**：診断本体ができたら `status` を `"soon" → "live"` に変えるだけでカードがリンク有効化。
- **母屋に載る仕組み**：`js/home.js` が `home.contents` をループ生成。静的HTMLにも同内容をフォールバックで置き、JS非実行のクローラにも見せる（GEO/AEO）。
- **アイコン**：`assets/type-<id>.svg`（128×128 角丸・パレット準拠）をローカル同梱。既存の `type-roomdry/baby/kitchen/bath/dani` に倣う。
- **sitemap**：公開した診断URLを `sitemap.xml` に1行追記し `lastmod` を更新（§9でSearch Consoleへ再送信）。
- **現在の登録**：`sentaku`（live）／`dani`（soon＝`/dani/` 実装完了後に live へ切替）。
...
## 8. 生成画像・写真の差し替え方法
- 初期は `assets/` の**オリジナルSVG**（すべてローカル同梱・完全オリジナル）。
  - **母屋専用**：`hero-hub.svg`（母屋ヒーロー＝家＋診断カルテ＋水滴キャラの世界観）／`ogp-hub.svg`（母屋OGP 1200×630）。**`/sentaku/` 等が使う `hero.svg`/`ogp.svg` とは別ファイルにして共存**（片方を差し替えても他方に影響しない）。
  - 診断アイコン：`type-roomdry/baby/kitchen/bath/dani.svg`。
- 写真を使う場合は **`assets/photos/` にローカル同梱**し、**`images.css` の変数（`--photo-*`）のファイル名を差し替えるだけ**。外部URLのホットリンクは禁止。
- 母屋のヒーロー/OGPを写真版にする場合は `hero-hub.svg`/`ogp-hub.svg` を差し替え、`index.html` の hero `img` と `og:image`/`twitter:image` のパスを合わせる（OGPはSNS互換のため **PNG 1200×630 推奨**：`ogp-hub.png` を追加して参照）。
